  ...
  return (
    <aside className="m-4 w-[23.5%] min-w-[250px] max-w-[700px] bg-[#2b3f64] text-white flex flex-col p-6 shadow-2xl rounded-3xl overflow-y-auto">
      ...
      <nav className="flex-1 space-y-6">
        ...
        <Link
          href="/"
          className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 font-semibold ${isActive("/")
              ...
              <div className="space-y-1 ml-2">
                {filteredSgd.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`flex items-center gap-3 px-4 py-2.5 rounded-2xl transition-all duration-300 text-sm font-medium ${isActive(item.href)
