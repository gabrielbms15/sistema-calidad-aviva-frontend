...
export default function GuiaTecnicaView({
  ...
  return (
    <div className="flex flex-col h-full items-center justify-center font-sans">
      ...
      <div className="w-full max-w-7xl mb-6 flex flex-col items-start">
      ...
      <div className="w-full max-w-7xl flex h-[75vh] min-h-[500px] bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-200">
        {/* Sidebar */}
        <aside className="w-64 shrink-0 bg-[#0f1117] flex flex-col border-r border-white/5">
          {/* Top spacing for sidebar to look clean since header was moved */}
          ...
          >
            <p className="text-[10px] uppercase tracking-widest text-white/25 mb-3 px-2">
              Macroprocesos
            </p>
            {macroprocesos.map((macro) => {
              ...
              return (
                ...
                >
                  <span className={`font-bold text-sm tracking-wide mb-1 ${isActive ? "text-blue-300 drop-shadow-md" : "text-white/70"}`}>
                    {macro.orden}. {macro.codigo}
                  </span>
                  <span className={`text-[11px] leading-snug ${isActive ? "text-white/90" : "text-white/40"}`}>
                    {macro.nombre}
                  </span>
                </button>
              );
          ...
          <div className="bg-white border-b border-gray-100 flex flex-col shrink-0">
            <div className="px-8 py-4 flex items-center justify-between">
              <h2 className="text-gray-800 font-bold text-lg leading-tight">
                {macroActual?.orden}. {macroActual?.nombre}
              </h2>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className="appearance-none bg-white border border-gray-200 text-gray-700 text-sm rounded-lg pl-3 pr-8 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300 cursor-pointer transition-all w-32"
                  >
            ...
            {/* Subheader: Descripcion del estandar seleccionado */}
            {selectedCodigoId && (() => {
              const selectedCodigoObj = codigos.find(c => c.id === selectedCodigoId);
              return selectedCodigoObj ? (
                <div className="bg-blue-50/50 px-8 py-3 border-t border-gray-100 text-sm text-gray-600">
          ...
          >
            {isPending ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                {/* Table header */}
                <div className="grid grid-cols-[180px_1fr] border-b border-gray-100 bg-gray-50/70">
                  <div className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                    Criterio
                  </div>
                  <div className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-gray-400 border-l border-gray-100 flex items-center justify-between">
                    <span>{isVerificadoresMode ? "Verificadores" : "Descripcion"}</span>
                    <button
                      onClick={() => setIsVerificadoresMode(!isVerificadoresMode)}
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${isVerificadoresMode ? 'bg-blue-500' : 'bg-gray-300'
                        }`}
                    >
                      <span
                ...
                {criteriosFiltrados.length === 0 ? (
                  ...
                  criteriosFiltrados.map((criterio, i) => (
                    <div
                      key={criterio.id}
                      className={`grid grid-cols-[180px_1fr] hover:bg-blue-50/50 transition-colors duration-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-200/45"
                    ...
                    >
                      ...
                      <div className="px-5 py-4 border-l border-gray-50 flex items-center">
                        {!isVerificadoresMode ? (
      ...
      <div className="w-full max-w-7xl mt-4 flex justify-end">
        <Link
          href="/acreditacion"
