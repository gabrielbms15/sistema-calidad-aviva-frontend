...
import AsignarResponsablesView from "./AsignarResponsablesView";

export default async function AsignarResponsablesPage() {
  ...
  const { data: criteriosIniciales } =
    codigoIds.length > 0
      ? await supabase
          .from("criterio")
          .select(
            "id,codigo_criterio,descripcion,codigo_id,fuente_0,fuente_1,fuente_2,criterio_responsable(id,criterio_id,responsable_id)"
          )
          .in("codigo_id", codigoIds)
      : { data: [] };

  return (
    <AsignarResponsablesView
