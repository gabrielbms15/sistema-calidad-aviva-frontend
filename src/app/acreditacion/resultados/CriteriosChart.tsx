...
/* ─── Colors (same palette as EntregablesChart) ──────────── */
 ...
 *  - Sin cumplido ni parcial, y TODOS con estado asignado
 *    son "no_cumplido"                                    → no_cumplido
 *  - Cualquier otro caso (todos null, o mezcla sin cumplido/parcial
 *    pero con algún null)                                 → sin_estado
 */
function deriveCriterioStatus(cr: CriterioEntregable): StatusKey {
  let hasCumplido    = false;
  let hasParcial     = false;
  let hasNoCumplido  = false;
  let allSinEstado   = true;

  for (const ent of cr.entregables) {
    if (ent.estado === "cumplido")         { hasCumplido   = true; allSinEstado = false; }
    else if (ent.estado === "parcial")     { hasParcial    = true; allSinEstado = false; }
    else if (ent.estado === "no_cumplido") { hasNoCumplido = true; allSinEstado = false; }
    // null: no contribution
  }

  if (cr.entregables.length === 0 || allSinEstado) return "sin_estado";
  if (hasCumplido)  return "cumplido";
  if (hasParcial)   return "parcial";

  // "no_cumplido" solo si no hay ningún null mezclado — es decir, todos
  // los entregables con estado asignado son no_cumplido y ninguno es null.
  const todosAsignados = cr.entregables.every((e) => e.estado !== null);
  if (hasNoCumplido && todosAsignados) return "no_cumplido";

  return "sin_estado";
}

/* ─── Helper: compute bar data ───────────────────────────── */
    ...
    .map((m) => {
      ...
      for (const est of m.estandares) {
        for (const cr of est.criterios) {
          ...
          const status = deriveCriterioStatus(cr);
          if (status === "cumplido")    cumplido++;
          else if (status === "parcial")     parcial++;
          else if (status === "no_cumplido") no_cumplido++;
          else sin_estado++;
        }
      }
...
function StackedBarRow({
  ...
  return (
    <div className="group flex items-center gap-4 py-[1px] px-3 rounded-xl transition-all duration-200 border border-transparent hover:border-[#2D778B]/10 hover:bg-[#2D778B]/[0.04] bg-transparent relative z-10">
      {/* Label */}
      ...
      <div className="flex-1">
        {total === 0 ? (
          <div className="h-5 bg-black/[0.03] rounded flex items-center justify-center ring-1 ring-inset ring-black/[0.06]">
            <span className="text-[10px] text-gray-300 font-medium">Sin criterios</span>
          </div>
        ) : (
...
/* ─── Main Component ─────────────────────────────────────── */
export default function CriteriosChart({ procesos, dataByProceso, selectedProcesoId }: CriteriosChartProps) {
      ...
      <div className="w-full lg:w-[70%] bg-white/80 backdrop-blur-2xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 flex flex-col ring-1 ring-black/[0.02]">
        ...
        <div className="px-8 pt-6 pb-2 flex items-center justify-between gap-4 shrink-0">
          <h2 className="text-[#000000] font-avenir-demi text-base font-bold tracking-tight">
            Criterios por Macroproceso
          </h2>
          <span className="text-[10px] text-gray-400 font-avenir leading-tight text-right max-w-[260px]">
            Estado derivado del avance de sus entregables
          </span>
        </div>
